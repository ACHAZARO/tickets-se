-- 078: si los dos nombres traen medidas distintas, el par es 'presentacion' aunque uno figure como sinonimo del
-- otro ("FRESA 907 G" tenia a "FRESA 454 G" de sinonimo y se ofrecia unificar: son dos tamanos, no el mismo articulo).
...
par AS (
  -- Si los dos nombres traen medidas DISTINTAS manda 'presentacion' (4), aunque uno sea sinonimo del otro:
  -- "FRESA 907 G" y "FRESA 454 G" no son el mismo articulo, son dos tamanos del mismo insumo.
  SELECT cand.x, cand.y,
         CASE WHEN bool_or(cardinality(pa.nums) > 0 AND cardinality(pb.nums) > 0 AND pa.nums <> pb.nums)
              THEN 4 ELSE min(cand.rango) END AS rango
  FROM cand
