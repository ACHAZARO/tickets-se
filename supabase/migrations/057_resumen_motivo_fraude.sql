-- 057: el motivo "fraude" del resumen = rechazado que esta en la revision de Fraude (sospechoso y no descartado),
-- igual que `estaEnFraude` del admin. Antes un papel repetido (rechazado + Fraude) salia como "duplicado".
-- "duplicado" queda para la misma FOTO subida dos veces (sin sospecha); "otro" = ilegible / manual.
