-- 058: ajustes tras la revision independiente de 056/057.
--  1) El monto del ORIGINAL solo se presta a un duplicado que NO esta confirmado (subido/por revisar/rechazado).
--     Si alguien confirma un duplicado sin monto propio, cuenta $0 como oficial (antes sumaba el original otra vez).
--  2) El total "todas las sucursales" solo incluye sucursales activas (igual que la API `por_sucursal` y el selector del admin).
--  3) El motivo del rechazo y el resto no cambian.
