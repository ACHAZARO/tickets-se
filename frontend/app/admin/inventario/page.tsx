...
interface Fila {
  nombre: string
  esInsumo: boolean            // la fila es un INSUMO que agrupa varias presentaciones (migracion 076)
  presentaciones: string[]     // nombres de los productos que suma
...
export default function EntradasPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    for (const row of (data as unknown as Array<{ descripcion: string | null; cantidad: number | null; unidad: string | null; monto: number | null; categorias_gasto: { nombre: string } | null; catalogo_productos: { nombre: string; unidad_default: string | null; contiene_cantidad: number | null; contiene_unidad: string | null; contiene_sub_cantidad: number | null; contiene_sub_unidad: string | null; insumos: { nombre: string; unidad_base: string } | null } | null }>) ?? []) {
      ...
      const etiqueta = insumo?.nombre?.trim() || nombre
      const key = (insumo ? 'i:' + insumo.nombre.toLowerCase() : 'p:' + nombre.toLowerCase())
      const f = map.get(key) ?? { nombre: etiqueta, esInsumo: !!insumo, presentaciones: [] as string[], categoria: null as string | null, veces: 0, cantidad: 0, unidad: null as string | null, unidadMixta: false, base: 0, baseUnidad: null as string | null, gasto: 0 }
      if (insumo && !f.presentaciones.includes(nombre)) f.presentaciones.push(nombre)
  ...
  function exportarCSV() {
    const head = ['Producto', 'Presentaciones', 'Categoria', 'Veces', 'Cantidad', 'Unidad', 'Unidades base', 'Unidad base', 'Gasto']
    const rows = filtradas.map(f => [f.nombre, f.presentaciones.join(' · '), f.categoria ?? '', f.veces, f.cantidad, f.unidadMixta ? 'mixta' : (f.unidad ?? ''), f.base || '', f.baseUnidad ?? 'Revisar', f.gasto])
